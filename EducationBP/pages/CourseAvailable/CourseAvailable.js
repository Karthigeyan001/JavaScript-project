(function () {
    ...
    ui.Pages.define("/pages/CourseAvailable/CourseAvailable.html", {
        ...
        computeItemSize: function (index) {
            ...
            if (index != null) {
                ...
                if (typeof (index) === "number") {
                    if (CourseData.items._groupedItems[index] != undefined) {
                        if (CourseData.items._groupedItems[index].data.recordType === RecordType.bigger) {
                            size.width = 343;
                            size.height = 455;
                        }
                        else if (CourseData.items._groupedItems[index].data.recordType === RecordType.big) {
                            size.width = 795;
                            size.height = 136;
                        }
                        else if (CourseData.items._groupedItems[index].data.recordType === RecordType.medium) {
                            size.width = 165;
                            size.height = 300;
                        }
                        else if (CourseData.items._groupedItems[index].data.recordType === RecordType.small) {
                            size.width = 167;
                            size.height = 222;
                        }
                        else if (CourseData.items._groupedItems[index].data.recordType === RecordType.smaller) {
                            size.width = 140;
                            size.height = 145;
                        }
        ...
        _initializeLayout: function (listView, viewState) {
            ...
            if (viewState === appViewState.snapped) {
                ...
                listView.itemDataSource = CourseData.groups.dataSource;
                listView.groupDataSource = null;
                listView.layout = new ui.ListLayout();
            } else {
                listView.itemDataSource = CourseData.items.dataSource;
                listView.groupDataSource = CourseData.groups.dataSource;
        ...
        _itemInvoked: function (args) {
            if (appView.value === appViewState.snapped) {
                ...
                var group = CourseData.groups.getAt(args.detail.itemIndex);
